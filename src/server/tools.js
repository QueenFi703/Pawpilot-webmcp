...
const petIdSchema = {
  type: 'string',
  description: 'Stable pet identifier. Dojo is dojo-001.',
  minLength: 1,
...
export const tools = [
  {
    ...
    description: 'Retrieve Dojo’s pet profile, including basic care information and preferences.',
    inputSchema: { type: 'object', properties: { petId: petIdSchema }, additionalProperties: false },
    annotations: { readOnlyHint: true, openWorldHint: false }
  },
  {
    name: 'get_daily_needs',
    description: 'Get Dojo’s care checklist. petId defaults to dojo-001 and date defaults to today.',
    inputSchema: {
      type: 'object',
      properties: {
        petId: petIdSchema,
        date: { type: 'string', description: 'Date in YYYY-MM-DD format. Defaults to today.', pattern: '^\\d{4}-\\d{2}-\\d{2}$', default: getToday() }
      },
  ...
  {
    name: 'save_care_plan',
    description: 'Persist an approved care plan. confirmed must be true only after the human explicitly approves the proposed plan.',
    inputSchema: {
      type: 'object',
      properties: {
        petId: petIdSchema,
        plan: {
          type: 'object',
          properties: { title: { type: 'string', minLength: 1, maxLength: 120 }, tasks: { type: 'array', items: { type: 'object' } }, services: { type: 'array', items: { type: 'object' } }, products: { type: 'array', items: { type: 'object' } } },
  ...
  },
  {
    ...
    description: 'List Dojo’s saved care plans, newest first. petId defaults to dojo-001.',
    inputSchema: { type: 'object', properties: { petId: petIdSchema }, additionalProperties: false },
    annotations: { readOnlyHint: true, openWorldHint: false }
  }
];
...
function validateParams(toolName, params) {
  ...
  for (const [key, property] of Object.entries(schema.properties || {})) {
    ...
    if (property.type === 'string' && typeof value !== 'string') throw new ToolInputError(`${key} must be a string`);
    if (property.type === 'boolean' && typeof value !== 'boolean') throw new ToolInputError(`${key} must be a boolean`);
    if (property.enum && !property.enum.includes(value)) throw new ToolInputError(`${key} must be one of: ${property.enum.join(', ')}`);
  ...
  if (toolName === 'save_care_plan') {
    const { plan, confirmed } = params;
    if (confirmed !== true) throw new ToolInputError('Human confirmation is required before saving a care plan.');
    if (typeof plan.title !== 'string' || !plan.title.trim()) throw new ToolInputError('plan.title is required');
    if (!Array.isArray(plan.tasks)) throw new ToolInputError('plan.tasks must be an array');
    for (const collection of ['services', 'products']) if (plan[collection] !== undefined && !Array.isArray(plan[collection])) throw new ToolInputError(`plan.${collection} must be an array`);
  }
}
...
function getDailyNeeds(petId, date) {
  if (petId !== miloProfile.id) return { success: false, error: 'Pet not found' };
  return { success: true, data: { ...dailyNeeds, date: date || new Date().toISOString().slice(0, 10) } };
}
