...
const tools = [
  {
    ...
    parameters: { type: 'object', properties: { petId: { type: 'string' } }, required: ['petId'], additionalProperties: false },
    strict: true,
  },
  {
    type: 'function',
    ...
    parameters: { type: 'object', properties: { petId: { type: 'string' }, date: { type: 'string', description: 'Optional YYYY-MM-DD date.' } }, required: ['petId', 'date'], additionalProperties: false },
    strict: true,
  },
  {
    type: 'function',
    ...
    parameters: { type: 'object', properties: { serviceType: { type: 'string', enum: ['veterinary', 'grooming', 'training', 'boarding'] } }, required: ['serviceType'], additionalProperties: false },
    strict: true,
  },
  {
    type: 'function',
    ...
    parameters: { type: 'object', properties: { category: { type: 'string', enum: ['food', 'treats', 'toys', 'bedding'] } }, required: ['category'], additionalProperties: false },
    strict: true,
  },
  {
    type: 'function',
    ...
    parameters: {
      type: 'object',
      properties: { petId: { type: 'string' }, plan: { type: 'object', additionalProperties: true }, confirmed: { type: 'boolean' } },
...
async function runTool(name, args) {
  if (name === 'save_care_plan' && args.confirmed !== true) {
    return { success: false, requiresConfirmation: true, error: 'Human confirmation is required before saving a care plan.' };
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let input = [
      ...history.slice(-10).map((item) => ({ role: item.role === 'assistant' ? 'assistant' : 'user', content: item.text })),
    ...
    for (let turn = 0; turn < 5; turn += 1) {
      ...
      if (functionCalls.length === 0) {
        return res.status(200).json({ success: true, text: response.output_text, toolCalls, model: process.env.OPENAI_MODEL || 'gpt-5.6-luna' });
      }
